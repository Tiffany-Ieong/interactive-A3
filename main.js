// PAGE TURNING
// the page turning function is referencing this youtube video:https://youtu.be/0kD6ff2J3BQ?si=in4orvxzq70eiccH
...
// the reference video is base on clicking so ill have to change it to dragging
// when the user presses down on the book, record the starting x position and begin tracking
book.addEventListener("mousedown", (e) => {
    dragStartX = e.clientX
    isDragging = true // set the flag to true so JS know a drag is happening
})
...
book.addEventListener("mouseup", (e) => {
    ...
    if (distance > 50) {
        ...
        goPrevPage()
    }// if the user dragged right more than 50px, go back to the previous page
    isDragging = false
})// reset when the drag is finished

book.addEventListener("mouseleave", () => {
    isDragging = false
})// if the mouse leaves the book area, cancel the drag
...
}// shoft the book sitting in the centered when it is closed

// turning forward the next page
function goNextPage() {
    if (currentLocation < maxLocation) {
        switch (currentLocation) {
            case 1:
                ...
                // these two lines were added as there is bug that paper pops up and block the cover before the turing animation ends
...
function goPrevPage() {
    if (currentLocation > 1) {
        ...
        switch (currentLocation) {
            case 2://unflip paper1 to go back to the cover
                closeBook(true);
            ...
            case 4:
                ...
                paper1.style.display = 'none'
                 // hide paper2 so it doesn't block the flip animation
                 // this is new from the tuttorial as the same bug happens that
...
// this is the reference youtube tutorial for the sticker dragging fuction, https://youtu.be/ymDjvycjgUM?si=3oNMNE0sWxnHY9EC
...
//startX and StartY record where the sticker is when the page first load
// newX and newY helps calculate the distance if the new and old position
...
stickers.forEach(sticker => {
    sticker.addEventListener('mousedown', mouseDown)// start the drag
...
function mouseDown(e) {
    ...
    startX = e.clientX
    startY = e.clientY
    // record whthe x and y coordinates of the mouse position at the moment of clicking
...
function mouseUp(e) {
    ...
    // stop tracking mouse movement — the drag is over
...
//new added so the sticker can stick to the page not floating on top of the book, alsp hide and flip with the attched page
    if (activeSticker) {
    ...
    if (currentPage) {
        ...
        if (
            ...
            if (activeSticker.classList.contains('sticker-wrapper')) {
                ...
                // re-select it so the outline and handle reappear for the player
                selectWrapper(activeSticker)
                setTimeout(() => {
                    activeSticker.dataset.ready = 'true'
                }, 100)
                // small delay before click-to-lock activates so the drop doesn't immediately lock it
            } else {
...
// reference: https://medium.com/the-z/making-a-resizable-div-in-js-is-not-easy-as-you-think-bda19a1bc53d
...
// lockSticker removes the selected class — CSS handles hiding the outline and handle automatically
function lockSticker() {
    if (selectedWrapper) {
        ...
        // reset — nothing is selected anymore
    }
}

// selectWrapper adds the selected class — CSS handles showing the outline and handle automatically
...
document.addEventListener('click', (e) => {
    if (selectedWrapper &&
        ...
        // contains checks if the click was inside the wrapper — if not, lock it
        selectedWrapper.dataset.ready === 'true') {
        // dataset.ready prevents locking immediately after placing — gives the player a moment
        lockSticker()
    }
})
...
function createStickerWrapper(stickerImg, parentPage) {
    ...
    // create the wrapper div — class handles all appearance via CSS
    const wrapper = document.createElement('div')
    wrapper.className = 'sticker-wrapper'
    // only size and position stay in JS since they are different for every sticker
    ...
    handle.addEventListener('mousedown', (e) => {
        ...
        function onResizeMove(e) {
            ...
            // update the wrapper size — the sticker inside scales with it automatically
